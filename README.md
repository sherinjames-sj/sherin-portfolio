...
2. At the domain registrar, add a `CNAME` DNS record (for a subdomain like `www`) or `A` records (for an apex domain) pointing at GitHub Pages — see GitHub's custom domain docs for the exact records.
